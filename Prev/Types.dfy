...
type Set<T(==)> {
  ...
  /*
  method {:axiom} Subset(e:Set<T>, ghost counter_in:nat) returns (b:bool, ghost counter_out:nat)
    ensures  b == (e.Model() <= Model()) 
    ensures counter_out == counter_in + Size()
  */
...
/*
lemma in_universe_then_correct(S:Set, U:Set)
requires in_universe_Set(S, U)
ensures correct_Set(S)
